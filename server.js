	...
	think(input) {
		...
		if (this.targetLock) {
			...
			if ((e.health.amount > 0) &&
				...
				(this.body.aiSettings.parentView || ((e.x - m.x) * (e.x - m.x) < sqrRange && (e.y - m.y) * (e.y - m.y) < sqrRange)) &&
				(this.body.aiSettings.skynet || ((e.x - mm.x) * (e.x - mm.x) < sqrRangeMaster && (e.y - mm.y) * (e.y - mm.y) < sqrRangeMaster)) &&
				(!this.body.aiSettings.ignoreBase || room.isNotInBase(e))) {} else {
				this.targetLock = undefined
				this.tick = 100
			}
		}
...
class Entity {
	...
	define(set) {
		...
		if (set.passive != null) {
			this.damageRecieved = 0, set.passive
		}
		if (set.NAME != null) {
			this.name = set.NAME;
		}
			...
			function incoming(message, socket) {
				...
				switch (m.shift()) {
						...
						if (chat_system == true) {
							if (!socket.status.deceased) {
								if (socket.status.playermuted == false) {
									...
									socket.talk('m', 'youre muted!')
								}
							}
						} else {
							socket.talk('m', 'chat system is disabled')
						};
						break;
						// =================================================================================
					case 'p':
					...
					case 'L':
						{ // level up cheat
							if (m.length !== 0) {
								socket.terminate('Ill-sized level-up request.');
								return 1;
							}
							// cheatingbois
							if (player.body != null) {
								if (player.body.skill.level < c.SKILL_CHEAT_CAP || player.body.skill.level < 60) {
					...
					case '0':
						{ // developer cheat
							if (m.length > 1) {
								socket.kick('Ill-sized developer request.');
								return 1;
							}
							// cheatingbois
							let tokens = {
                        ...
                        `.trim().split('\n').reverse())
								...
								case 88: //[x] selfbot cheat
									player.body.define({
										CONTROLLERS: ['minion', 'nearestDifferentMaster']
									})
									break;
								case '186': // [;] heal
						...
						case "2tdm":
							{
								...
								for (let i = 0, m = 0; i < 2; i++) {
									let v = Math.round(1000000 * (room['dom' + (i + 1)].length + 1) / (census[i] + 1) / scoreCensus[i]);
									if (v > m) {
										m = v;
										possiblities = [i];
									} else if (v == m) {
								...
								if (room['dom' + player.team].length)
									do {
										loc = room.randomType('dom' + player.team);
...
var maintainloop = (() => {
	...
	function placeRoids() {
		...
		for (let i = Math.ceil(roidcount); i; i--) {
			count++;
			placeRoid('roid', Class.obstacle2);
		}
		for (let i = Math.ceil(roidcount * 0.3); i; i--) {
			count++;
			placeRoid('roid', Class.babyObstacle2);
		}
		for (let i = Math.ceil(rockcount * 0.8); i; i--) {
			count++;
			placeRoid('rock', Class.obstacle2);
		}
		for (let i = Math.ceil(rockcount * 0.5); i; i--) {
			count++;
			placeRoid('rock', Class.babyObstacle2);
		}
	...
	let placemediumWalls = () => {
		...
		for (let loc of room['mall']) {
			let o = new Entity({
				x: loc.x + room.xgridWidth / 2,
				y: loc.y + room.ygridHeight / 2
			})
		...
		return census => {
			if (timer > 4000 && ran.dice(13000 - timer)) {
				...
				switch (ran.chooseChance(30, 15, 20, 10, 9, 7, 3, 3, 3)) {
					case 0:
						choice = [
							[Class.elite_destroyer, Class.elite_battleship, Class.elite_gunner, Class.elite_sprayer], 1, 'a', 'nest'
						];
						break;
					case 1:
						choice = [
					...
					case 2:
						choice = [
							[Class.elite_destroyer, Class.elite_battleship, Class.elite_gunner, Class.elite_sprayer], 2, 'a', 'nest'
						];
						break;
					case 3:
						choice = [
					...
					case 4:
						choice = [
							[Class.palisade, Class.nestKeeper, Class.skimboss, Class.summoner], 1, 'a', 'norm'
						];
						sockets.broadcast('A strange trembling...');
						break;
					case 5:
						choice = [
							[Class.palisade, Class.nestKeeper, Class.skimboss, Class.summoner], 2, 'a', 'norm'
						];
						sockets.broadcast('A strange trembling...');
						break;
					case 6:
...
let server = http.createServer((req, res) => {
	...
	switch (pathname) {
		case '/':
			res.writeHead(200)
			res.end(`<!DOCTYPE html><h3>US East</h3><h3>Heroku Web Server</h3><button onclick="location.href = 'http://arras.io/#host=' + location.host">Launch</button>`)
			break
		case '/mockups.json':
