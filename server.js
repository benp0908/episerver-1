...
const MazeGenerator = class {
  ...
  clear(mapString) {
    if (mapString) {
      ...
      this.maze = Array(32).fill().map((_, y) => Array(32).fill().map((_, x) => map[x][y]))
    } else {
  ...
  erodeSym4(side, corner) {
    ...
    if (this.staticRand < 0.5) {
      this.maze[x][y] = false
      this.maze[x][31 - y] = false
      this.maze[31 - x][y] = false
      this.maze[31 - x][31 - y] = false
    } else {
      this.maze[x][y] = false
      this.maze[y][31 - x] = false
      this.maze[31 - x][31 - y] = false
      this.maze[31 - y][x] = false
    }
  }
  ...
  placeMinimal() {
    ...
    for (let i = 0; i < 1; i++) {
      ...
      if (best === null || squares.length < best.length) {
        best = squares
        continue
      }
    }

    for (let i = 0; !best && i < 500; i++) {
      let trial = this.runTrial()
      if (!trial) continue
...
let maze = new MazeGenerator(11)
let runCode = () => {
  try {
    let minimal = maze.placeMinimal()
    s.style.color = '#006600'
    s.textContent = `Success (${ maze.isClosed() ? 'closed' : 'open' })`
  
    let pixelSize = 10
    c.width = pixelSize * 32
    c.height = pixelSize * 32
    let ctx = c.getContext('2d')
    ctx.scale(pixelSize, pixelSize)
    ctx.fillStyle = '#fff'
    ctx.fillRect(0, 0, 32, 32)
    ctx.fillStyle = '#000'
    for (let { x, y, size } of minimal)
